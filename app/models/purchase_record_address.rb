class PurchaseRecordAddress
  ...
  with_options presence: true do
    ...
    validates :item_id
    validates :token
    validates :postal_code, format: { with: /\A[0-9]{3}-[0-9]{4}\z/, message: 'に(-)が含まれていません。' }
    validates :city, format: { with: /\A[ぁ-んァ-ン一-龥]/, message: 'は全角でご入力ください。' }
    validates :house_number
    validates :telephone_number, format: { with: /\A\d{10}\z|\A\d{11}\z/ }
  end
  validates :prefecture_id, numericality: { other_than: 1, message: "は空欄にできません。" }
